...
function printReport(buttonElement) {
    if (!resultsContent) {
        console.error("Results content not found.");
        if (buttonElement) alert("Error: Report content not available for printing.");
        return;
    }
    ...
    titleElement.innerText = "The Emperor's New Code - Analysis Report";
    titleElement.style.textAlign = 'center';
    titleElement.style.fontFamily = '"Instrument Serif", serif'; 
    titleElement.style.fontSize = '24px'; 
    titleElement.style.marginBottom = '20px';
    printableArea.appendChild(titleElement);
    
    const clonedResultsContent = resultsContent.cloneNode(true);
    printableArea.appendChild(clonedResultsContent);
    ...
    setTimeout(() => {
        ...
        if (buttonElement && buttonElement.closest('.modal-footer')) {
            closeStickerModal();
        }
    }, 100); 
}
